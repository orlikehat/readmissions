...
def f_preprocess_blood_count():
    path = "C:/Users/orlyk/readmissions/project/preprocessed/labs/blood_count/blood_count_preprocessed"
    files = os.listdir( path )
    df_pop=pd.read_pickle(r"C:\Users\orlyk\readmissions\project\preprocessed\population\df_readmin_with_labels_base_slim.pkl")
    l_casenum=df_pop["CaseNum"].drop_duplicates()
    df_blood_count=l_casenum
...
#                    df_fin=df_fin.drop(columns=["temp_first","temp_min","temp_last","temp_max"])
    ...
    for file in files:
        ...
        df_blood_count=pd.merge(df_blood_count,df,how="outer",on="CaseNum")
        
        
    #add engineered features:##################
    df_blood_count_ef=df_blood_count.dropna()
    #add NLR
    ...
    df_blood_count_results_only= df_blood_count.filter(regex=r'(result|CaseNum)')
    
    df_blood_count.to_pickle(r"C:\Users\orlyk\readmissions\project\preprocessed\labs\blood_count\lab_blood_count_for_model\blood_count_results_with_dates.pkl")
    df_blood_count_results_only.to_pickle(r"C:\Users\orlyk\readmissions\project\preprocessed\labs\blood_count\lab_blood_count_for_model\blood_count_results_only.pkl")
    
    return df_blood_count_results_only
